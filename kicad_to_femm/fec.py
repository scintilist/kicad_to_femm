    ...
    """ Subclass must:
        ...
        Subclass may:
            -check if it has already been initialized when __init__ is called using the 'initialized' flag.
                if it has, it can merge the new arguments into the existing instance.
...
class Point(Multiton):
    """ Point is a multiton, but points are put into 1e-3mm (1um) square bins. """
    instances = WeakKeyDictionary()  # Strong reference held by the segment that contains it
    ...
    def __key__(self):
        if not self._cached_key:
            self._cached_key = round(self.x * 1e3 + Point.X_GRID_OFF), round(self.y * 1e3 + Point.Y_GRID_OFF)
...
class Segment(Multiton):
    """ Segment is a multiton with strong references (must explicitly remove segments from the instance dict). """
    instances = {}
...
def _resolve_overlaping_segments():
    """ Process segments to remove overlaps.
        ...
        First splits segments in two wherever they are very nearly touching a point.
        No new points are created, the endpoints will then be the point they almost touched.
        This may result in overlapping segments.
    """
    # Get a list of all points
    points = list(Point.instances.keys())

    # Get a list of all segments, then clear the segment dictionary
    segments_to_process = list(Segment.instances)
    Segment.instances = {}

    # Pop segments and check for intersections until none left to process.
    while segments_to_process:
        segment = segments_to_process.pop()

        line = gm.LineString([(p.x, p.y) for p in segment.points])
        unbroken = True
        for split_point in points:
            if (gm.Point(split_point.x, split_point.y).distance(line) < 1e-3) and (split_point not in segment.points):
                print('split!')
                # Line broken by point, and the point is not one of it's endpoints.
...
def write_out(file_name, precision=1e-8, frequency=0, min_angle=25, thickness=.035,
    ...
    with open(file_name, 'w') as f:
        ...
        _merge_close_points()
        _resolve_overlaping_segments()
        _remove_zero_length_segments()

        # Force garbage collection to ensure there are no orphaned objects in the weak dictionaries
        gc.collect()
